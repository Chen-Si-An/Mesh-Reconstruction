...
Pivoter::Pivoter(const pcl::PointCloud<pcl::PointNormal>::Ptr &_cloud, const double _ballRadius)
{
...
#ifdef SUP_GPU
	gpuCloud.resize(_cloud->size());
	for (size_t i = 0; i < _cloud->size(); i++)
	{
		pcl::PointNormal point = _cloud->points[i];
		gpuCloud[i].x = point.x;
		gpuCloud[i].y = point.y;
		gpuCloud[i].z = point.z;
		gpuCloud[i].nx = point.normal_x;
		gpuCloud[i].ny = point.normal_y;
		gpuCloud[i].nz = point.normal_z;
	}
	...
	if (useGPU = GpuUtils::setDevice(0))
	{
		GpuRoutines::prepareStackSize();
		GpuRoutines::buildInDeviceKDTree(&gpuCloud);
	}
#else
	useGPU = false;
#endif // SUP_GPU
...
TrianglePtr Pivoter::findSeedCPU()
//Allen_20221006A_Ball Pivoting使用GPU加速
{
	//DebugLevel debug = Config::getDebugLevel();	//Allen_20221003A_支援BPA點雲轉STL
	...
	//std::cout << "Remaining points " << notUsed.size() << std::endl;	//Allen_20221003A_支援BPA點雲轉STL

	//Allen_20221003A_支援BPA點雲轉STL
	//for (std::map<int, bool>::iterator it = notUsed.begin(); it != notUsed.end() && !found; it++)
	for (std::map<int, bool>::iterator it = notUsed.begin(); it != notUsed.end(); it++)
	//Allen_20221003A_支援BPA點雲轉STL
	{
		int index0 = it->first;
...
std::vector<int> Pivoter::getNeighbors(const pcl::PointNormal &_point, const double _radius) const
{
	...
	//Allen_20221006A_Ball Pivoting使用GPU加速
#ifdef SUP_GPU
	if (0)	//此處先不使用GPU，經測試效能較差，overhead都在memory copy
	{
		gpu::Point gpuPoint;
		gpuPoint.x = _point.x;
		gpuPoint.y = _point.y;
		gpuPoint.z = _point.z;
		GpuRoutines::radiusSearch(gpuPoint, &gpuCloud, _radius, indices);
	}
	else
#endif // SUP_GPU
	//Allen_20221006A_Ball Pivoting使用GPU加速
...
TrianglePtr Pivoter::findSeedGPU()
{
	...
	for (std::map<int, bool>::iterator it = notUsed.begin(); it != notUsed.end() && !found; it++)
	{
		...
		// Look for a valid seed
		gpu::BallCenter center = GpuRoutines::findSeed(&gpuCloud, indices, notUsedArray, index0, ballRadius);
